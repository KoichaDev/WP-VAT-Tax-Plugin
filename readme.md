...
⚠️ Make sure to run the code below, otherwise you will just get a blank screen on the `Calculcate Tax/VAT` plugin
...
## Conventional Prefix in PHP

I have used the prefix on PHP code for function callback. It start with `gs_`. The `gs` stands for "Godt Sagt". It's just a "best practice" to do that in WP. This is due to if users download many plugins that could have for example a function name that is `add_number()`. WP will send an error and "crash" the website. It's better to safe for example `gs_add_number()` instead. 
...
The end result actually working fine as the application for the React, but for developing experience, it wasn't all that good in my opinion. You could look for example at [this commit](https://github.com/KoichaDev/WP-VAT-Tax-Plugin/tree/adffd9b99dd3c64e35147a3cb4b8b4311670552d/form-calculating) here to see the "end result" of the `useState`. It's not 100% fine-tuned compared to the latest git commit when I decided to use the Context API with `useReducer` . 
